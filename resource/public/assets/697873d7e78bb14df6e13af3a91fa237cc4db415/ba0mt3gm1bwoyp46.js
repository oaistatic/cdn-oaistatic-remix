import{r as s,cH as jn,f4 as W,G as qn,cz as Wn,hM as Hn,hw as Gn,eZ as Xn,hn as $n,g as Kn,e9 as H,m as d,aE as Dn,bL as Tn,L as Rn,b6 as M,ah as fn,dx as Yn,dm as Zn,D as Jn,bn as mn,co as Qn,b7 as ne}from"./dfm2yceao9m2ilo9.js";import{lQ as Fn,lR as ee,i as te,lS as oe,gN as ae,fq as se,j0 as G,lT as ie,i9 as vn,ia as B,d2 as re,bB as le,aS as ce,j3 as gn,i$ as pn,lU as ue,ic as de}from"./o7wih49jz3zkdcav.js";import{u as fe}from"./c5yzhyi4mbwizere.js";import{X as me,k as ve,l as ge,u as pe}from"./giv27hat5yu275a3.js";import{m as be}from"./ccrdffu4uq6bo5k0.js";const xe=2,he=({viewport:t,canvasSize:n,shouldMeasurePerf:e,source:i})=>{const o=s.useRef({}),a=s.useCallback(r=>{const f=r.getExtension("WEBGL_debug_renderer_info");f&&(o.current.vendor=r.getParameter(f.UNMASKED_VENDOR_WEBGL),o.current.renderer=r.getParameter(f.UNMASKED_RENDERER_WEBGL))},[]);return s.useEffect(()=>{const r={vendor:o.current.vendor??"(unavailable)",renderer:o.current.renderer??"(unavailable)",width:t[0],height:t[1],canvasWidth:n[0],canvasHeight:n[1],dpr:window.devicePixelRatio,source:i};!jn(r,o)&&e&&(o.current=r,Fn(xe).then(g=>{W.bloop.performance({...r,fps:g})}))},[n,e,t,i]),a},Se=.25,ye=3,bn=28,U=55,O=200,xn=1.1,Ae=({viewport:t,initialScale:n,shouldCalibrate:e})=>{const[i,o]=s.useState(n),a=s.useRef(void 0),r=s.useRef(void 0),f=s.useRef(void 0),g=s.useRef("down"),p=s.useCallback(()=>{const c=g.current==="up"?ye:Se;Fn(c).then(u=>{if(u<bn){g.current="down";const m=Math.max(u,1),l=Math.sqrt(m/bn);o(x=>x*l),r.current=window.setTimeout(p,O)}else if(f.current){const m=u>=U,l=u>=f.current*xn;if(m||l)g.current="up",o(x=>x*xn),r.current=window.setTimeout(p,O);else if(u<U){g.current="down";const x=Math.sqrt(u/U);o(b=>b*x)}}f.current=u})},[]);return s.useEffect(()=>{const[c,u]=t,[m,l]=a.current??[];return e&&(m!==c||l!==u)&&(a.current=t,clearTimeout(r.current),o(n),r.current=window.setTimeout(p,O)),()=>{clearTimeout(r.current)}},[e,p,n,t]),i},Ce=60,P=Math.floor(1e3/Ce),we=(t,n)=>{const[e,i]=s.useState(t===n?performance.now():0),o=s.useRef(n);s.useEffect(()=>{o.current!==n&&t===n&&i(performance.now()),o.current=n},[n,t]);const a=s.useCallback(()=>{i(0)},[]);return[t===n?1:0,e,a]},De=([t,n,e],i=500)=>{const[o,a]=s.useState(t),r=s.useRef(t),f=s.useRef(void 0),g=s.useCallback((p,c,u)=>{const m=l=>{const x=Math.min((l-p)/i,1),b=c+x*(u-c);a(b),x<1&&(f.current=requestAnimationFrame(m))};f.current=requestAnimationFrame(m)},[i]);return s.useEffect(()=>{if(r.current!==t){r.current=t,f.current&&cancelAnimationFrame(f.current);const p=o,c=t;g(performance.now(),p,c)}},[g,o,t]),[o,n/1e3,e]};var Te=qn,Re=Wn;function Fe(t){return Re(t)&&Te(t)}var Me=Fe,ke=Hn,_e=Xn,ze=$n,Ee=Gn,Pe=Me,Ne=Math.max;function Le(t){if(!(t&&t.length))return[];var n=0;return t=ke(t,function(e){if(Pe(e))return n=Ne(e.length,n),!0}),Ee(n,function(e){return _e(t,ze(e))})}var Ie=Le,Be=ee,Ue=Ie,Oe=Be(Ue),Ve=Oe;const V=Kn(Ve),X=240,Mn=2048,kn=new Array(X).fill(0),je=t=>{const n=me(t,{bands:X,updateInterval:P,loPass:0,hiPass:400,analyserOptions:{fftSize:Mn}});return n.length===0?kn:n},$=t=>{const{magnitudes:n,binCount:e,gainMultiplier:i}=t;if(n.length===0)return new Array(e).fill(0);const o=Math.ceil(n.length/e),a=[];for(let r=0;r<n.length;r+=o){const f=Math.min(r+o,n.length),g=n.slice(r,f).sort((m,l)=>m-l),p=Math.floor(g.length/2);let c;g.length%2===0?c=(g[p-1]+g[p])/2:c=g[p],c=Math.abs(c),c*=i;const u=c/(c+1);a.push(u)}return a},_n=(t,n)=>{const e=n.sampleRate,i=t.length,o=n.bandCount,a=n.binCount,r=n.gainMultipliers;if(r.length!==o)throw new Error("gainMultipliers must have length equal to bandCount");const f=i*2,g=e/f,p=t.map((x,b)=>b*g),c=20,u=e/2,m=new Array(o+1).fill(0).map((x,b)=>c*Math.pow(u/c,b/o)),l=new Array(o).fill(null).map(()=>[]);for(let x=0;x<p.length;x+=1)for(let b=0;b<o;b+=1)if(p[x]>=m[b]&&p[x]<m[b+1]){l[b].push(t[x]);break}for(let x=0;x<o;x+=1){const b=l[x],v=r[x];l[x]=$({magnitudes:b,binCount:a,gainMultiplier:v})}return l},hn=60,zn=48e3,qe=1,We=2,He=40,Ge=2;function Sn({time:t,timeConstant:n}){return 1-Math.exp(-t/n)}function yn(t,n){const e=n[1]-n[0];return n[0]+t*e}const j=t=>{if(!Array.isArray(t)||t.length!==2||typeof t[0]!="number"||typeof t[1]!="number")throw new Error("zip failed - received a value that is not a [number, number] tuple")},Xe=t=>{const{prevAudioData:n,prevCumulativeAudioData:e,deltaTimeS:i,audioDataRaw:o}=t,a=o.map(m=>m*i*hn*qe),r=Sn({time:i,timeConstant:We}),f=V(n,a).map(m=>(j(m),yn(r,m))),g=o.map(m=>m*i*hn*He),p=V(e,g).map(m=>(j(m),m[0]+m[1])),c=Sn({time:i,timeConstant:Ge}),u=V(e,p).map(m=>(j(m),yn(c,m)));return{audioData:f,cumulativeAudioData:u}},_t=(t,n)=>{const e=je(t),i=_n(e,{sampleRate:zn,binCount:n.bins,bandCount:n.bands,gainMultipliers:n.gainMultipliers}),o=$({magnitudes:e,binCount:1,gainMultiplier:1});return{bandMagnitudes:i,cumulativeMagnitude:o}},$e="webp",Ke=512,Ye=512,Ze="srgb",Je=3,Qe="uchar",nt=72,et=!1,tt=8,ot=!1,at=!1,st="{{.assetPrefix}}/assets/noise-watercolor-m3j88gni.webp",it={format:$e,width:Ke,height:Ye,space:Ze,channels:Je,depth:Qe,density:nt,isProgressive:et,paletteBitDepth:tt,hasProfile:ot,hasAlpha:at,src:st};let K;const E=new window.Image;E.crossOrigin="anonymous";E.src=it.src;E.onload=()=>{K=E};function rt(t){const n=s.useRef(t);n.current=t;const[e,i]=s.useState(null),o=s.useCallback(a=>{i(a)},[]);return H(()=>{const a=new ResizeObserver(r=>{for(const f of r)n.current(f)});return e&&a.observe(e),()=>{a.disconnect()}},[e]),o}const R=ve.createChild("GLCanvas");function lt({className:t,vert:n,frag:e,Behaviors:i,onViewportUpdate:o,onRenderComplete:a,onGlAvailable:r,onCanvasSizeUpdate:f,scale:g}){const p=s.useRef(null),c=s.useRef(null),[u,m]=s.useState(null),[l,x]=s.useState(),b=s.useRef(te(h=>{const{width:S,height:C}=h.contentRect,y=Math.floor(Math.min(S??0,C??0));m(y),o?.({width:y,height:y})},100)),v=rt(b.current);if(H(()=>{const h=p.current,{width:S,height:C}=h??{};if(h&&u!=null&&S&&C){const y=h.getContext("webgl2",{premultipliedAlpha:!0});y?x(y):R.error("webgl2 context not supported")}},[u]),H(()=>{if(!l)return;const h=l.createProgram(),S=l.createShader(l.VERTEX_SHADER),C=l.createShader(l.FRAGMENT_SHADER);if(!h){R.debug("failed to create program");return}if(!S||!C){R.debug("failed to create shaders",S,C);return}let y="";if(l.shaderSource(S,n),l.compileShader(S),y+=`vertShader:
