import{r as l,z as Rn,e6 as B,w as Fn,c2 as Mn,fW as _n,fX as kn,dF as En,fP as Pn,g as zn,cQ as O,j as f,bC as mn,ak as In,ck as Ln,ca as Nn,D as Un,aB as _,aF as en,bv as Bn,J as On}from"./e9yuj0ll30e3kmlx.js";import{ak as Vn,J as jn}from"./mhi0wfvgf6n45t1b.js";import{u as qn}from"./emf898bco4g2v57x.js";import{l0 as gn,l1 as Wn,l as Gn,l2 as Hn,kb as Xn,cL as $n,k as tn,bt as Kn,aG as Yn,dd as on,dc as an,l3 as Zn}from"./ihoxwwe79kyl4jef.js";import{X as Jn,k as Qn,l as ne,a as ee}from"./h8hvc5v41a0lczjj.js";import{m as te}from"./ogeaiwpkr6sm1oz0.js";const oe=2,ae=({viewport:t,canvasSize:n,shouldMeasurePerf:e,source:i})=>{const o=l.useRef({}),c=l.useCallback(r=>{const m=r.getExtension("WEBGL_debug_renderer_info");m&&(o.current.vendor=r.getParameter(m.UNMASKED_VENDOR_WEBGL),o.current.renderer=r.getParameter(m.UNMASKED_RENDERER_WEBGL))},[]);return l.useEffect(()=>{const r={vendor:o.current.vendor??"(unavailable)",renderer:o.current.renderer??"(unavailable)",width:t[0],height:t[1],canvasWidth:n[0],canvasHeight:n[1],dpr:window.devicePixelRatio,source:i};!Rn(r,o)&&e&&(o.current=r,gn(oe).then(p=>{B.bloop.performance({...r,fps:p})}))},[n,e,t,i]),c},se=.25,ie=3,sn=28,z=55,I=200,rn=1.1,re=({viewport:t,initialScale:n,shouldCalibrate:e})=>{const[i,o]=l.useState(n),c=l.useRef(void 0),r=l.useRef(void 0),m=l.useRef(void 0),p=l.useRef("down"),g=l.useCallback(()=>{const v=p.current==="up"?ie:se;gn(v).then(d=>{if(d<sn){p.current="down";const u=Math.max(d,1),s=Math.sqrt(u/sn);o(a=>a*s),r.current=window.setTimeout(g,I)}else if(m.current){const u=d>=z,s=d>=m.current*rn;if(u||s)p.current="up",o(a=>a*rn),r.current=window.setTimeout(g,I);else if(d<z){p.current="down";const a=Math.sqrt(d/z);o(h=>h*a)}}m.current=d})},[]);return l.useEffect(()=>{const[v,d]=t,[u,s]=c.current??[];return e&&(u!==v||s!==d)&&(c.current=t,clearTimeout(r.current),o(n),r.current=window.setTimeout(g,I)),()=>{clearTimeout(r.current)}},[e,g,n,t]),i};var le=Fn,ce=Mn;function ue(t){return ce(t)&&le(t)}var de=ue,fe=_n,me=En,ge=Pn,ve=kn,pe=de,he=Math.max;function be(t){if(!(t&&t.length))return[];var n=0;return t=fe(t,function(e){if(pe(e))return n=he(e.length,n),!0}),ve(n,function(e){return me(t,ge(e))})}var xe=be,Se=Wn,Ae=xe,ye=Se(Ae),Ce=ye;const L=zn(Ce),V=240,vn=2048,we=60,j=Math.floor(1e3/we),pn=new Array(V).fill(0),De=t=>{const n=Jn(t,{bands:V,updateInterval:j,loPass:0,hiPass:400,analyserOptions:{fftSize:vn}});return n.length===0?pn:n},q=t=>{const{magnitudes:n,binCount:e,gainMultiplier:i}=t;if(n.length===0)return new Array(e).fill(0);const o=Math.ceil(n.length/e),c=[];for(let r=0;r<n.length;r+=o){const m=Math.min(r+o,n.length),p=n.slice(r,m).sort((u,s)=>u-s),g=Math.floor(p.length/2);let v;p.length%2===0?v=(p[g-1]+p[g])/2:v=p[g],v=Math.abs(v),v*=i;const d=v/(v+1);c.push(d)}return c},hn=(t,n)=>{const e=n.sampleRate,i=t.length,o=n.bandCount,c=n.binCount,r=n.gainMultipliers;if(r.length!==o)throw new Error("gainMultipliers must have length equal to bandCount");const m=i*2,p=e/m,g=t.map((a,h)=>h*p),v=20,d=e/2,u=new Array(o+1).fill(0).map((a,h)=>v*Math.pow(d/v,h/o)),s=new Array(o).fill(null).map(()=>[]);for(let a=0;a<g.length;a+=1)for(let h=0;h<o;h+=1)if(g[a]>=u[h]&&g[a]<u[h+1]){s[h].push(t[a]);break}for(let a=0;a<o;a+=1){const h=s[a],y=r[a];s[a]=q({magnitudes:h,binCount:c,gainMultiplier:y})}return s},ln=60,bn=48e3,Te=1,Re=2,Fe=40,Me=2;function cn({time:t,timeConstant:n}){return 1-Math.exp(-t/n)}function un(t,n){const e=n[1]-n[0];return n[0]+t*e}const N=t=>{if(!Array.isArray(t)||t.length!==2||typeof t[0]!="number"||typeof t[1]!="number")throw new Error("zip failed - received a value that is not a [number, number] tuple")},_e=t=>{const{prevAudioData:n,prevCumulativeAudioData:e,deltaTimeS:i,audioDataRaw:o}=t,c=o.map(u=>u*i*ln*Te),r=cn({time:i,timeConstant:Re}),m=L(n,c).map(u=>(N(u),un(r,u))),p=o.map(u=>u*i*ln*Fe),g=L(e,p).map(u=>(N(u),u[0]+u[1])),v=cn({time:i,timeConstant:Me}),d=L(e,g).map(u=>(N(u),un(v,u)));return{audioData:m,cumulativeAudioData:d}},ut=(t,n)=>{const e=De(t),i=hn(e,{sampleRate:bn,binCount:n.bins,bandCount:n.bands,gainMultipliers:n.gainMultipliers}),o=q({magnitudes:e,binCount:1,gainMultiplier:1});return{bandMagnitudes:i,cumulativeMagnitude:o}};function ke(t){const n=l.useRef(t);n.current=t;const[e,i]=l.useState(null),o=l.useCallback(c=>{i(c)},[]);return O(()=>{const c=new ResizeObserver(r=>{for(const m of r)n.current(m)});return e&&c.observe(e),()=>{c.disconnect()}},[e]),o}const R=Qn.createChild("GLCanvas");function Ee({className:t,vert:n,frag:e,Behaviors:i,onViewportUpdate:o,onRenderComplete:c,onGlAvailable:r,onCanvasSizeUpdate:m,scale:p}){const g=l.useRef(null),v=l.useRef(null),[d,u]=l.useState(null),[s,a]=l.useState(),h=l.useRef(Gn(b=>{const{width:S,height:x}=b.contentRect,C=Math.floor(Math.min(S??0,x??0));u(C),o?.({width:C,height:C})},100)),y=ke(h.current);if(O(()=>{const b=g.current,{width:S,height:x}=b??{};if(b&&d!=null&&S&&x){const C=b.getContext("webgl2",{premultipliedAlpha:!0});C?a(C):R.error("webgl2 context not supported")}},[d]),O(()=>{if(!s)return;const b=s.createProgram(),S=s.createShader(s.VERTEX_SHADER),x=s.createShader(s.FRAGMENT_SHADER);if(!b){R.debug("failed to create program");return}if(!S||!x){R.debug("failed to create shaders",S,x);return}let C="";if(s.shaderSource(S,n),s.compileShader(S),C+=`vertShader:
